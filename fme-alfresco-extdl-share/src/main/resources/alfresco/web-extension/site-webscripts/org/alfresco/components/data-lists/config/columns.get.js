...
function main()
{
   ...
   for (var i = 0; i < columns.length; i++) {
      //note: this would be fine if it didn't default to the opposite of OOTB (sorted=false)
      //columns[i].sorted = "" + formConfig.getFields()[columns[i].name].sorted;
      if (formConfig.getFields()[columns[i].name].attributes["disableSort"] != null) {
         columns[i].disableSort = formConfig.getFields()[columns[i].name].attributes["disableSort"];
      }
   }
   // HMH - Ixxus - END
